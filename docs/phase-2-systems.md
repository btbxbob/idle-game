...
本节作为当前阶段重构的系统级约束，优先级高于旧的 phase 2 描述。

### 0.1 内容揭示规则

1. 未揭示内容不能进入前端返回结果。
2. 未揭示内容不能显示名称。
3. “锁定但已知”和“完全未知”必须分开处理：
   - 完全未知：不返回给前端。
   - 已揭示但未解锁：返回正式名称、条件和状态。
4. 这一规则适用于阶段、建筑、科技、解锁项。

### 0.2 阶段系统职责

Rust 侧必须新增显式阶段模型，至少覆盖以下状态：
...
人口相关系统除了已有的饥饿、死亡、尸体转化，还要扩展出共生模型：
...
2. 尸体开始衰变并产生蛆虫。
3. 系统标记“黑暗分支已揭示”。
4. 前端才首次收到蛆虫相关解锁项、建筑和科技名称。

### 0.5 代码落点

建议实现落点如下：

- `src/state/`：阶段状态、共生状态、保存字段
- `src/systems/`：阶段判断、揭示过滤、共生数值更新
- `src/core/idle_game.rs`：主循环接入、WASM 导出、建筑/科技查询过滤
- `js/unlocks.js`：只渲染 Rust 返回的已揭示条目
- `js/technology-manager.js` / 建筑渲染层：只展示已揭示内容
...
| 铁锭冶炼厂 | 10 IronOre | 1 IronIngot | 10:1 |

#### 工厂化生产规则

- 不再依赖独立合成系统；加工资源全部改为建筑持续产出。
- 次级资源建筑必须消耗初级资源后才能产出。
- 高级资源建筑必须同时消耗初级资源和次级资源后才能产出。
- T1 科技成本只能使用初级资源，避免玩家在基础阶段被次级/高级资源卡住。
