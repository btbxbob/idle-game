...
test('get_statistics is callable from JavaScript and returns correct data', async ({ page }) => {
  ...
  const totalClicks = await page.evaluate(() => {
    const stats = (window.rustGame.getStatistics ? window.rustGame.getStatistics() : window.rustGame.get_statistics());
  ...
  expect(updatedTotalClicks).toBeGreaterThanOrEqual(3);
