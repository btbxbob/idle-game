...
test.describe('Monkey Test Suite - 高优先级测试 (必须覆盖)', () => {
    ...
    test('rapid clicking should not crash game', async ({ page }) => {
        ...
        for (let batch = 0; batch < 5; batch++) {
            const batchPromises = [];
            for (let i = 0; i < 100; i++) {
                batchPromises.push(page.click('#coin-button'));
            }
...
test.describe('Monkey Test Suite - 中优先级测试 (推荐覆盖)', () => {
    ...
    test('rapid language switching during gameplay', async ({ page }) => {
        ...
        for (let i = 0; i < 15; i++) {
            ...
            // 交替切换语言
            if (i % 3 === 0) {
                await page.locator('#language-select-setting').isVisible()
                    .then(async (isVisible) => {
                        if (isVisible) {
                            await page.selectOption('#language-select-setting', 'en');
                        }
                    }).catch(() => {});
            } else if (i % 3 === 2) {
                await page.locator('#language-select-setting').isVisible()
                    .then(async (isVisible) => {
                        if (isVisible) {
                            await page.selectOption('#language-select-setting', 'zh-CN');
                        }
                    }).catch(() => {});
            }
    ...
    test('simultaneous operations with reasonable concurrency', async ({ page }) => {
        ...
        for (let i = 0; i < 10; i++) {
            actionPromises.push(page.click('#coin-button'));
            if (i % 2 === 0) {
...
test.describe('Monkey Test Suite - 低优先级测试 (可选覆盖)', () => {
    ...
    // 测试 10: 升级购买与手动点击混合测试
    // ============================================================================
    test('upgrade purchase and manual click interaction', async ({ page }) => {
        await waitForGameInitialization(page);
        ...
        if (enhancedCPC > originalCPC) {
            ...
            // 现在尝试购买第二个升级（当前为 Lumberjack Efficiency）
            for (let i = 0; i < 20; i++) {
                await page.click('#coin-button');
            }
            
            console.log('尝试购买升级 #1 ...');
            await page.click('#buy-upgrade-1');
            ...
            console.log("无法购买Better Click，因为资源不足，跳过该交互测试");
        }
        
        // 验证最终状态一致性
        await verifyGameStateConsistency(page);
        console.log('✓ 升级与点击混合测试完成');
    });
